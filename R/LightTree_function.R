...
LightTree=function(TagMatrix, #the input matrix, per row cells, per column gene
    ...
    if(is.null(coordinates)){
        ...
        if(PCAdim!=0){
            pca_result=prcomp(TagMatrix,center = TRUE,scale. = TRUE)
            pca_coordinate=pca_result$x[,1:PCAdim]
            rtsne_result=Rtsne(pca_coordinate,dims=3,max_iter = 2500)
            rtsne_coordinate=rtsne_result$Y
        }else{
            rtsne_result=Rtsne(TagMatrix,dims=3,max_iter = 2500)
            rtsne_coordinate=rtsne_result$Y
        }
    ...
    #PART II
    #cell belonging
    cellbelong=data.frame(timepoint=libTimePoint$timeorder[match(cellbelong,libTimePoint$lib)],
    ...
    if(is.null(total_state_num)){
        properK=round(nrow(TagMatrix)/50*2)
    }else{
    ...
    cellbelong$timepoint_state_kmean=kmean_result$cluster
    ...
    for(j in 1:properK){
        ...
        cellbelong$state_center_cell_id[tagcell]=center_cell
    ...
    for(i in 1:timepoint_num){
        ...
        timepoint_centers=c(timepoint_centers,starting_cell)
    ...
    timepoint_centers=data.frame(timepoint=timepoint,
                                 state=timepoint_state,
                                 center=timepoint_centers,
                                 prec_time=prec_time,
