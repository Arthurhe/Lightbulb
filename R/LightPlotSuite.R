...
plotting_lineage_states=function(coord,timepoint_center,cluster_label,lib_label,endpoint_ret=NULL,lineage_color_touse="set1"){
  require(gridExtra)
  ...
  state_center_cood=data.frame(state=timepoint_center$state,
                               coord_x=coord[timepoint_center$center,1],
                               coord_y=coord[timepoint_center$center,2],stringsAsFactors=F)
  starting_cell=timepoint_center[which(timepoint_center$prec_center==0),,drop=F]
  timepoint_center=timepoint_center[which(timepoint_center$prec_center!=0),,drop=F]
  
  #convert the shit to coord
  starting_cell=data_tab[starting_cell$center,,drop=F]
  mapping_table=data.frame(cbind(coord[timepoint_center$prec_center,],coord[timepoint_center$center,]))
  ...
  if(is.null(endpoint_ret)){
    #plot black tree
    g1=g1+ ggplot2::geom_segment(data=mapping_table,aes(x=start_x, xend=stop_x, y=start_y, yend=stop_y), size = 0.8,inherit.aes =F,arrow = arrow(length = unit(0.2, "cm")))
  }else{
    #plot tree by lineage color
    #get the lineage
    segment_col=colorRampPalette(RColorBrewer::brewer.pal(min(9,length(endpoint_ret$unique_timeline)), "Set1"))(length(endpoint_ret$unique_timeline))
    the_order_gp=unlist(endpoint_ret$unique_timeline)
    coltouse=c()
    for(i in 1:length(endpoint_ret$unique_timeline)){
      coltouse=c(coltouse,rep(segment_col[i],length(endpoint_ret$unique_timeline[[i]])))
    }
    coltouse=coltouse[match(timepoint_center$state,the_order_gp)]
    g1 =  g1 + 
      ggplot2::geom_segment(data=mapping_table,aes(x=start_x, xend=stop_x, y=start_y, yend=stop_y),
                            colour="black",size = 3,inherit.aes =F,arrow = arrow(length = unit(0.33, "cm"))) + 
      ggplot2::geom_segment(data=mapping_table,aes(x=start_x, xend=stop_x, y=start_y, yend=stop_y),
                            colour=coltouse,size = 0.8,inherit.aes =F,arrow = arrow(length = unit(0.2, "cm")))
  }
...
plotting_state_center=function(coord,timepoint_center,cluster_label){
  #create the master dataframe
  data_tab=data.frame(x=coord[,1],y=coord[,2],
                      state=factor(cluster_label,levels=unique(cluster_label)))
  #plot the dots
  s1=ggplot2::ggplot(data_tab, aes(x, y,colour=state)) + 
    ggplot2::geom_point(size=1) + 
    ggplot2::geom_text(data=data.frame(coord[timepoint_center$center,]),aes(X1, X2,label=timepoint_center$state),inherit.aes=F) +
    ggplot2::theme(legend.position="none")
